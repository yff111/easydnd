import type { DragDropEventType, DragDropOptions, DragDropPayload, DropPosition, DropPositionRules, GetElementIdFn, GetRectFn, Rect } from './types'

export function makePayloadFactory(opts: DragDropOptions) {
  return (
...
export function calcPosition(allowedPositions: DropPositionRules, threshold: number = 0.3) {
  return (offset: number = 0, widthOrHeight: number = 0): DropPosition => {
    switch (allowedPositions) {
      case 'all':
        return offset < widthOrHeight * threshold ? 'before' : offset > widthOrHeight * (1 - threshold) ? 'after' : 'in'
      case 'notAfter':
        return offset < widthOrHeight * threshold ? 'before' : 'in'
      case 'around':
        return offset < widthOrHeight / 2 ? 'before' : 'after'
...
export function resolveSelectedElements(
  ...
  return selected.includes(element) ? selected : [...selected, element]
}
...
export function reorderItems<T = any>(source: T[], items: T[], targetIndex: number) {
  ...
  copy.splice(targetIndex, 0, items)
  return copy.filter(e => !items.includes(e)).flatMap(e => e) as T[]
}
...
export function moveItemsToArrayMutate<T = any>(source: T[], target: T[], items: T[], targetIndex: number) {
  const copy: any[] = [...source]
  target.splice(targetIndex, 0, ...items)
  return copy.filter(e => !items.includes(e)) as T[]
}
